...
class CNN_baseline(tf.keras.Model):
    ...
    def __init__(self):
        ...
        self.cnn_layer1 = tf.keras.layers.Conv1D(filters=self.nkernels[0],data_format='channels_first',kernel_size = self.conv_kernel_size,  padding='valid',activation='relu')
        self.maxpool1 = tf.keras.layers.MaxPool1D(pool_size=self.pool_kernel_size,data_format='channels_first',strides=self.pool_kernel_size, padding='valid')
        #self.dropout1 = tf.nn.dropout(0.2)
        
        self.cnn_layer2 = tf.keras.layers.Conv1D(filters=self.nkernels[1], data_format='channels_first',kernel_size = self.conv_kernel_size, padding='valid',activation='relu')
        self.maxpool2 = tf.keras.layers.MaxPool1D(pool_size=self.pool_kernel_size, data_format='channels_first',strides=self.pool_kernel_size, padding='valid')
        #self.maxpool2 = tf.nn.max_pool(ksize=pool_kernel_size, strides=pool_kernel_size)
        #self.dropout2 = tf.nn.dropout(0.2)
        
        self.cnn_layer3 = tf.keras.layers.Conv1D(filters=self.nkernels[2], data_format='channels_first',kernel_size = self.conv_kernel_size, padding='valid',activation='relu')
    ...
    def loss(self, logits, labels):
        ...
        #cross_entropy = tf.keras.backend.binary_crossentropy(labels, logits, from_logits=True)
        return loss
...
def train(model, train_inputs, train_labels):
    ...
    for i in range(int(train_inputs.shape[0]/model.batch_size)):
        print(i*model.batch_size, (i+1)*model.batch_size)
